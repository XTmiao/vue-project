(function(window,document){
    ...
    var app = {
        init:function(){
           document.addEventListener('DOMContentLoaded',function(){   //当初始的 HTML 文档被完全加载和解析完成之后
               points = document.querySelectorAll('.pagenumber div') ;
               app.bindTouchEvent(); //绑定触摸事件
               //app.bindBtnClick();  //绑定按钮点击事件
               app.BtnClick();
               app.setPageNow();     //设置初始页码
           }.bind(app), { passive: false });
        }(),


        BtnClick:function(){
            var button = document.querySelector('.pagenumber').getElementsByTagName('div');
            clearTimeout(timer)
            for( let i=0; i<button.length; i++ ){
              button[i].addEventListener( 'click', ()=>{
                viewport.style.webkitTransition = "0.3s ease -webkit-transform";
                this.transform.call(viewport, -window.innerWidth*i);
                $(button[i]).addClass('now').siblings().removeClass('now');
              } )
            }
        ...
        bindTouchEvent:function(){
           ...
           }.bind(this), { passive: false });   /*从 chrome56 开始，在 window、document 和 body 上注册的 touchstart 和 touchmove 事件处理函数，会默认为是 passive: true，浏览器会有警告*/
       }
    }
})(window,document);
